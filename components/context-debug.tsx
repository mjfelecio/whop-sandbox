  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-3xl flex-col gap-6 p-6 md:p-10">
      <div className="flex items-center gap-3">
        ...
        <Text color="gray" size="2">Whop runtime observation</Text>
      </div>

      <div className="space-y-2">
        <Heading as="h1" size="7">{title}</Heading>
        <Text color="gray">{description}</Text>
      </div>

      {message ? (
        <Card size="2">
          <Text color={allowed ? "gray" : "red"} size="2">{message}</Text>
        </Card>
      ) : null}

      <Card size="2">
        <Heading as="h2" size="4">Observed context</Heading>
